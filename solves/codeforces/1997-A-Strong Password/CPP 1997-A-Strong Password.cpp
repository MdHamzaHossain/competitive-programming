...
int main()
{
    testCase
    {
        ...
        if (ind == -1)
        {
            ...
            {
                ind = s.size() / 2;
                // cout << " DEBUG" << " " << (s[ind - 1] - 'a') + 1 % 25 << endl;
                char c = mp[((s[ind - 1] - 'a') + 1) % 25];
                if (s[ind] == c)
                {
                    // cout << " DEBUG" << " " << ((c - 'a' + 1) % 25) << mp[(s[ind - 1] - 'a') + 1 % 25] << endl;
                    c = mp[((c - 'a' + 1) % 25)];
                }
                s.insert(s.begin() + ind, c);
            }
        }
        else
        {
            s.insert(s.begin() + ind, mp[(s[ind] - 'a' + 1) % 25]);
        }
        cout << s;
        PRINTEND;
    }

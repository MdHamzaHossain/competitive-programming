...
int main()
{
    // testCase
    {
        // ki fau question bhairebhai
        ll s, v1, v2, t1, t2;
        SCANLL(s);  // num of chars
        SCANLL(v1); // first speed (per char)
        SCANLL(v2); // second speed (per char)
        SCANLL(t1); // first ping
        SCANLL(t2); // second ping
        ll f1 = s * v1 + t1 * 2;
        ll f2 = s * v2 + t2 * 2;
        PRINTSTR((f1 < f2) ? "First" : ((f2 < f1) ? "Second" : "Friendship"));
        PRINTEND;
    }

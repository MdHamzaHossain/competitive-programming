...
class SegmentTree {
  public: explicit SegmentTree(const vector < int > & nums, int k): n(nums.size()),
  k(k),
  tree(4 * n) {
    build(nums, 0, 0, n - 1);
  }
  ...
  private: const int n; // the size of the input array
...
class Solution {
  public: vector < int > resultArray(vector < int > & nums, int k,

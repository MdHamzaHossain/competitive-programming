...
import { updateLeetcodeMd } from "./leetcode/leetcodeUpdateMD.js";

export async function updateEveryMD() {
    const langChart: string[][] = [["Language", "Solves"]];
    const platforms = ["codeforces", "leetcode", "toph"];
    const platformData = [await updateCodeforcesMd(), await updateLeetcodeMd(), await updateTophMd()];

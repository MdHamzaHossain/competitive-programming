...
int main()
{
    ...
    for (auto &c : s)
    {
        ...
        if (c == 'o')
        {
            wSAfterOs.push_back(t);
        }
        prev = c;
    }
    ll sum = 0;
    reverse(b2e(wSAfterOs));
    FORN(i, 0, wSAfterOs.size())
    {
        // cout << "DEBUG " << i << "th O: before: " << wSBeforeOs[i] << " after: " << wSAfterOs[i] << endl;
        sum += wSAfterOs[i] * wSBeforeOs[i];
    }
    PRINTLL(sum);
    PRINTEND;
    return 0;

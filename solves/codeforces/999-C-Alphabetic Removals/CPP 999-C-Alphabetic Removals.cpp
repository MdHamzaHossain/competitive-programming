...
int main()
{
    ...
    FORN(i, 0, n)
    charTrack[s[i] - 'a'].PB(i);

    int chIndex;
    for (chIndex = 0; chIndex < 26; chIndex++)
    {
        if (k > 0 && (ll)charTrack[chIndex].size() <= k)
            k -= (ll)charTrack[chIndex].size();
        else
            break;
    }

    FORN(i, k, (ll)charTrack[chIndex].size())
    keep.PB(charTrack[chIndex][i]);

    FORN(i, chIndex + 1, 26)
    for (auto e : charTrack[i])
        keep.PB(e);
